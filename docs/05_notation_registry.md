...
| `AutoAttack-$\ell_\infty$` | `autoattack_linf` | $\ell_\infty$ | No | Yes | `report/chapters/chapter5/3-attack_suite.tex`, `tab:autoattack_config`; results in `tab:worst_autoattack_sanity` and `tab:autoattack_fulltest`. | Evaluated separately from the 20-seed paired aggregate panel. It appears as a 512-sample sanity check and as a full-test random-seed panel stress test; it is not part of Mean(8). |
