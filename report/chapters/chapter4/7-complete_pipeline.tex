...
AttackDRO++ removes this assumption by replacing fixed attack groups with latent
groups discovered from adversarial examples. Gradient fingerprints add
optimization-level information to the clustering features, while the uniform
anchor keeps the objective close to the strong multi-attack baseline. The
training protocol keeps source attacks fixed, excludes held-out attacks from
optimization, and reports robustness per attack and through aggregate metrics.
Cluster refresh and cluster-score updates are configurable implementation choices:
in the main experiments, clusters are refreshed every \(R=2\) epochs, \(\mathbf{q}\)
is reset after refresh, and \(s_k\) may come from cluster loss, difficulty score,
  ...
  {\textbf{Update Cluster Weights}\enspace
   ...
   $q_k \;\leftarrow\;
     \mathrm{Proj}_{q_{\min}}\!\!\left(
       q_k \cdot \exp\!\bigl(\eta_q\,s_k\bigr)\right)$};
...
Core training loop of AttackDRO++. A minibatch is converted into a mixed
PGD-\(\ell_\infty\)/DDN-\(\ell_2\) adversarial batch, passed through the model
to construct augmented clustering features \(\psi_i\), assigned to latent
clusters, and optimized with a uniform-anchored combination of ERM and
Cluster-DRO losses. Cluster weights \(\mathbf{q}\) and K-means centroids are
updated periodically according to the configured refresh and reweighting policy.%
}
...
\For{epoch \(e = 1,\ldots,E\)}{
    \For{each minibatch \((x,y)\) of size \(N\)}{
        ...
        Compute logits \(z \leftarrow f_\theta(x^{\mathrm{adv}})\) and features \(h \leftarrow \phi_\theta(x^{\mathrm{adv}})\)\;
        Build augmented clustering features \(\{\psi_i\}_{i=1}^N \leftarrow \mathrm{BuildFeature}(z_i,h_i,y_i,P)\)\;
        Assign cluster ids \(c_i \leftarrow \mathrm{AssignCluster}(\psi_i,\mathrm{Clusterer},K)\) for all samples\;
        ...
        \If{\(e > T_w\)}{
            Compute cluster scores \(s_k\) from the configured \(q\)-update metric\;
            Update \(\mathbf{q}\) using exponentiated gradient with \(s_k\) and floor projection\;
        }

        Add detached features \(\{\psi_i\}_{i=1}^N\) to feature bank \(\mathcal{F}\)\;
    }

    \If{refresh condition is met}{
        Refresh K-means clusterer using detached \(\psi_i\) in \(\mathcal{F}\)\;
...
intervention. The progression builds sequentially: Single-AT diagnoses the cross-attack
gap by training against one source attack, which leaves a large vulnerability to other
threat models. Uniform multi-attack ERM covers multiple source attacks with equal
weighting, providing a strong baseline but failing to adapt to weak groups. AttackDRO
introduces attack-level reweighting but assumes each attack identity is homogeneous.
To resolve this, the cluster-DRO component replaces fixed attack identities with
discovered latent groups. Gradient fingerprints enrich this grouping with
optimization-level information, and the uniform anchor stabilizes the adaptive objective
by preserving the strong multi-attack training signal. Together, these form the
final AttackDRO++ method.

The evaluation discipline from Stage 1 carries through the method design: source attacks are
...
The final implementation evaluated in the main experiments uses gradient
fingerprints, an online K-means refresh policy, \(K=4\) clusters, and
uniform-anchor strength \(\lambda_{\mathrm{DRO}}=0.35\). Chapter~\ref{chap:experimental_setup}
documents the full training configuration, software stack, and evaluation protocol.

The main additional state introduced by AttackDRO++ is the feature bank used for
cluster refresh. The bank stores detached augmented clustering features \(\psi_i\),
so clustering does not backpropagate through past minibatches. K-means is refitted
only at the configured refresh interval, and the cluster weights are updated using
the configured cluster score \(s_k\) before being reset or refreshed when cluster
identities change. This keeps the adaptive grouping mechanism tied to recent model
representations while avoiding a per-step clustering cost.

The dominant computational cost remains adversarial example generation, because
all compared multi-attack methods use the same mixed source attacks. The
AttackDRO++ additions therefore change the training objective and bookkeeping
rather than the threat model or evaluation protocol. Chapter~\ref{chap:experimental_setup}
specifies the concrete training configuration, software stack, and evaluation
procedures used to run this implementation.
...
\(q\) learning rate      & \(\eta_q\)                  & \(3\times10^{-4}\) & Step size for exponentiated-gradient update using \(s_k\) \\
...
Gradient weight          & \(\lambda_{\mathrm{grad}}\) & 0.1   & Weight of GradFP component in \(\psi_i\) \\
Label weight             & \(\lambda_{\mathrm{lbl}}\)  & 0.1   & Weight of scalar label component in \(\psi_i\) \\
