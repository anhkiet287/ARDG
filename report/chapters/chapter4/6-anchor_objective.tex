...
% TODO[Kiệt]:
% Goal:
% - Explain why the uniform anchor stabilizes adaptive Cluster-DRO.
% Flow:
% - Define the instability risk, then introduce anchor interpolation, q-floor, and warmup.
% - Connect each mechanism to a specific stability concern.
% Include:
% - Keep all hyperparameter values consistent with Chapter 5 tables.
% Avoid:
% - Presenting ablation conclusions here.
...
In the main experiments, we use \(\lambda_{\mathrm{DRO}}=0.35\), referred to as
the \texttt{Anchor35} setting. This value keeps the uniform multi-attack objective as
...
\begin{equation}
    ...
    \label{eq:floor}
\end{equation}
This prevents any cluster from being completely ignored. In our default setting,
\(q_{\min}=0.03\). Together, the uniform anchor and the floor projection ensure that
training receives both a stable per-sample ERM signal and an adaptive cluster-level
correction.
