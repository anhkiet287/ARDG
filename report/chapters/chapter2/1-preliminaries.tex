...
% TODO[ÄA]:
% Goal:
% - Explain notation and the learning setting needed for the rest of the report.
% Flow:
% - Define classification notation, model notation, loss, dataset, and perturbation notation.
% - Keep symbols consistent with docs/05_notation_registry.md.
% Include:
% - A small notation table only if many symbols are introduced here.
% - A sentence that Chapter 4 gives the full AttackDRO++ formulation.
% Avoid:
% - Introducing result metrics or implementation details too early.
% Target length: 3-5 paragraphs plus optional notation table.
...
which maps an input $x\in\mathbb{R}^{d}$ to a vector of $K$ real-valued outputs. These outputs are commonly referred to as logits and represent the modelâ€™s unnormalized scores for each class. The predicted class is obtained by selecting the class with the largest logit:
...
% TODO[DA]:
% Goal:
% - Explain cross-entropy, empirical risk minimization, and why gradient-based optimization matters for attacks.
% Flow:
% - Define the training loss, then connect input gradients to adversarial examples.
% Include:
% - Keep symbols aligned with docs/05_notation_registry.md.
% Avoid:
% - Deep optimizer details that belong in Chapter 5.
...
% TODO[ÄA]:
% Goal:
% - Explain adversarial examples intuitively before the formal equations.
% Flow:
% - Clean image -> small perturbation -> adversarial image -> changed prediction.
% - Define perturbation budget, norm constraint, targeted vs untargeted setting, and label preservation.
% Include:
% - Consider a simple conceptual figure for clean image, perturbation, adversarial image, and prediction change.
% - Connect the idea to the cross-attack gap without repeating Stage 1.
% Avoid:
% - Long attack history or detailed derivations of every attack.
% Target length: 4-6 paragraphs plus optional figure.
...
\subsection{Lp-Norm Perturbation Sets}
...
The resulting adversarial example $x+\delta^\ast$ corresponds to an input within the allowed perturbation region that maximizes the modelâ€™s loss. In practice, this maximization is approximated by different attack algorithms, such as FGSM, PGD, Carlini--Wagner, DeepFool, or AutoAttack. Stage 1 used these attacks as a diverse evaluation suite to measure how robust a PGD-trained model remains when exposed to attacks beyond the one used during training.
...
% TODO[ÄA]:
% Goal:
% - Define whitebox, graybox, and blackbox access levels in a compact way.
% Flow:
% - Start with perturbation constraints, then explain model access assumptions.
% - Tie whitebox to direct evaluation/training and graybox to transfer evaluation.
% Include:
% - Consider a small table comparing access to model parameters, gradients, logits/scores, and labels.
% - State that Chapter 6 uses whitebox and graybox analysis; blackbox is background only unless explicitly evaluated.
% Avoid:
% - Making blackbox attacks a major focus if they are not used in the current experiments.
...
where $p\in\{2,\infty\}$ and $\varepsilon$ controls the perturbation magnitude. In addition to the perturbation constraint, the adversaryâ€™s access to the model determines the type of threat model. We distinguish three common settings: white-box, gray-box, and black-box attacks.
...
using multiple projected gradient ascent steps. Since the attack is generated using the current modelâ€™s gradients, adversarial training naturally operates in a white-box setting.
...
In this project, gray-box evaluation is useful for measuring transfer robustness. While white-box evaluation tests whether a model is robust to attacks generated directly against itself, gray-box evaluation tests whether robustness generalizes across source and target models. This is especially relevant to the attacks-as-domains perspective, because transfer attacks can be interpreted as another form of distribution shift: the perturbation is generated from one modelâ€™s loss landscape but evaluated on another modelâ€™s decision boundary.
