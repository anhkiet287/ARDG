...
% TODO[Kiệt]:
% Goal:
% - Explain the architecture only as much as needed for reproducibility and the method.
% Flow:
% - State ResNet-18 adaptation, logits, penultimate feature dimension, and why this supports clustering.
% Include:
% - Keep notation consistent with Chapter 4 and docs/05_notation_registry.md.
% Avoid:
% - Long architecture tutorial or unplanned architecture comparisons.
...
This feature vector is used for clustering adversarial examples in AttackDRO++ and for constructing the gradient-fingerprint augmented feature described in Chapter 4. For ResNet-18 on CIFAR-10, the penultimate representation dimension is (d\_h = 512). Therefore, the raw classifier-head gradient proxy has dimension (C d\_h = 10 \times 512 = 5120), which is later reduced by random projection.
