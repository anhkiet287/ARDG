...
Table~\ref{tab:ch5_tools_platforms} summarizes the main tools and platforms used
in the experimental workflow. Google Colab and Google Drive are used for
...
Figure~\ref{fig:ch5_wandb_core_metrics} shows representative W\&B exports for
the core tracking metrics and the adaptive group-weight trajectory used as an
AttackDRO++ diagnostic.
...
\centering
\includegraphics[width=\linewidth]{figures/wandb/wb_train_loss.png}
\caption{Training loss.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/wandb/wb_train_acc.png}
...
\begin{subfigure}[t]{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/wandb/wb_val_clean_acc.png}
\caption{Clean validation accuracy.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/wandb/wb_pgd_linf_acc.png}
...
\begin{subfigure}[t]{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/wandb/wb_ddn_l2_acc.png}
\caption{DDN-$\ell_2$ validation accuracy.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/wandb/media_images_media_q_trajectory_15650_2752bccd0ffdd754ab89.png}
\caption{Group-weight trajectory.}
\label{fig:ch5_q_trajectory}
\end{subfigure}
\caption{Representative W\&B dashboard exports for experiment tracking. Panels (a)--(e) track training loss, training accuracy, clean validation accuracy, and validation robustness under PGD-$\ell_\infty$ and DDN-$\ell_2$ during a training run. Panel (f) records the AttackDRO++ group-weight trajectory diagnostic.}
\label{fig:ch5_wandb_core_metrics}
\end{figure}
...
\paragraph{Summary.}
Table~\ref{tab:ch5_setup_summary} summarizes the full experimental setup used as
the basis for the result claims in Chapter~6.

\begin{table}[H]
\centering
\caption{Summary of the experimental setup used in this report. Unless otherwise stated, the main whitebox results use CIFAR-10, ResNet-18, five random seeds, and the eight non-AutoAttack evaluation attacks.}
...
Main compared methods
& PGD-AT, DDN-AT, Multi-AT, AttackDRO, AttackDRO++, and AttackDRO++ (Ours). \\
...
& Mean robust accuracy over the eight non-AutoAttack evaluation attacks. \\

Secondary metrics
& Worst robust accuracy over the same eight attacks, seen-source robustness, held-out $\ell_\infty$ robustness, held-out $\ell_2$ robustness, per-attack accuracy, per-class accuracy, and graybox transfer accuracy. \\

Repeated-seed protocol
& Five random seeds are used for the main repeated-seed comparisons. \\
