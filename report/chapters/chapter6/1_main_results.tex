...
Relative to Multi-AT, AttackDRO++ improves Mean(8) by $+0.279$~pp
($p = 0.0076$, 14/20 seed-level wins), with the largest channel on the
held-out $\ell_2$ mean ($+0.405$~pp, $p = 0.0079$). 

Worst(8) and
AutoAttack-512 are statistically indistinguishable from Multi-AT
($|\Delta| < 0.005$~pp, $p > 0.97$), so AttackDRO++ improves average and
held-out robustness while leaving the standardized hardest-case checks
broadly unchanged.
...
held-out $\ell_2$ mean. Each therefore dominates one threat family but
loses substantially on the other, while AttackDRO++ achieves the highest
Mean(8), held-out attack mean, and seen-source mean (the metrics that
average across both families).
...
Multi-AT. AttackDRO++ (Ours) is highest on 3 of 8 attacks; the strongest
...
improving average robustness balance rather than dominating every individual
...
AttackDRO++ runs in this panel exhibit lower seed-to-seed standard deviation
...
AttackDRO++ runs in this panel exhibit lower seed-to-seed standard deviation than
Multi-AT on average-case metrics. This suggests a more stable observed outcome,
although the present comparison does not isolate which component causes the
reduction. The effect is weaker for Worst(8) and AutoAttack, whose standard
deviations remain similar across methods. This is consistent with the previous
sections: the hardest \(\ell_\infty\) stress metrics are broadly unchanged
relative to Multi-AT, while the main benefit of AttackDRO++ is on average and
held-out robustness.
