...
Multi-AT. AttackDRO++ wins the highest
...
why the Mean(8) gain does not automatically imply a stronger Worst(8). AttackDRO++ achieves the highest
...
The pattern is consistent with a specialist
...
seen $\ell_\infty$ attack ($-0.003$~pp). The per-attack pattern explains
why the Mean(8) gain does not translate into a stronger Worst(8): the
improvement is concentrated on held-out attacks, while the weakest
evaluation direction remains broadly unchanged.
...
reduces seed-to-seed variation relative to Multi-AT. To check whether the gain is more
predictable across repeated runs, Table~\ref{tab:ch6_variance_reduction} compares the
standard deviation of each seed-level metric between the two methods.

\begin{table}[H]
\centering
\caption[Seed variance reduction]{Variance reduction across 20 paired random-seed runs. \(\sigma\) is the
...
This suggests that the anchored Cluster-DRO objective not only improves the mean of average-case robustness metrics, but also makes the Multi-AT training outcome less sensitive to random seed. The effect is weaker for Worst(8) and AutoAttack, whose standard deviations remain similar across methods, which is consistent with the broader interpretation from the previous sections: AttackDRO++ improves average and held-out robustness relative to Multi-AT while leaving the hardest standardized \(\ell_\infty\) stress checks broadly unchanged. Because aggregate and per-attack metrics can still hide which CIFAR-10 classes and attack-by-class cells drive the remaining failures, the next section drills down into class-wise whitebox robustness to test whether the observed gains are broadly distributed or concentrated in a small part of the evaluation grid.
