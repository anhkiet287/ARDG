...
achieves $2.5$--$3.3{\times}$ tighter seed-to-seed standard deviation than Multi-AT on every average-case metric. Worst(8) and AutoAttack-512 are
statistically indistinguishable from Multi-AT ($|\Delta| < 0.005$~pp,
$p > 0.97$): AttackDRO++ improves average and held-out robustness while
leaving the standardized hardest-case checks broadly unchanged. The single-attack baselines
...
seen $\ell_\infty$ attack ($-0.003$~pp), which explains why Worst(8)
remains broadly unchanged rather than improved.
...
hardest-case metrics. On the $n=20$ sanity panel, the paired differences are
$-0.003$~pp for Worst(8) ($p=0.971$) and $-0.001$~pp for AutoAttack-512
($p=0.998$). This full-test panel indicates that AttackDRO++ is statistically
indistinguishable from Multi-AT under full-test AutoAttack-$\ell_\infty$
in the selected five-seed panel ($-0.006$~pp, $p=0.9584$). The hardest-case
checks are therefore broadly comparable rather than improved: AttackDRO++
improves average and held-out robustness while leaving the standardized
hardest-case checks broadly unchanged.
