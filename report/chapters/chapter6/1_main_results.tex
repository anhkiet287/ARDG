...
held-out $\ell_2$ mean ($+0.405$~pp, $p = 0.0079$). Worst(8) and
...
\begin{table}[H]
\centering
\caption{Worst(8) and AutoAttack-512 sanity results across
...
and is reported separately from Mean(8). For AttackDRO++ $-$ Multi-AT,
the paired differences are $-0.003$~pp on Worst(8) ($p=0.971$) and
$-0.001$~pp on AutoAttack-512 ($p=0.998$). Bold values indicate the best method
for each metric.}
...
These checks should therefore be read as broadly comparable rather than
improved.
