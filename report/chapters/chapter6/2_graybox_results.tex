...
\label{sec:graybox_transfer_results}
...
The whitebox results in Section~\ref{sec:main_results} show that
...
Section~\ref{sec:graybox-design}. In this setting, adversarial examples
are generated on one checkpoint and evaluated on an independently trained
target checkpoint.

\subsection{Seed Selection and Whitebox Sanity Check}
\label{sec:graybox_panel}

% TODO[Kiệt]:
% Goal:
% - Justify the five-seed graybox subset and show it is consistent with the main whitebox trends.
% Flow:
% - Explain seed selection, then interpret `tab:graybox_whitebox_sanity`.
% Include:
...
The graybox panel uses five representative seeds,
\(\{73,\ 202,\ 1337,\ 7777,\ 12345\}\), applied to all four methods. This
subset was chosen to match the full \(n=20\) paired deltas within
\(0.022\) pp on every reported metric, while also including seed \(7777\),
the worst AttackDRO++ seed in the full panel. Thus, the panel is a
representative computational subset rather than a favorable one.
...
from Table~\ref{tab:agg-results}.}
\label{tab:graybox_whitebox_sanity}
\small
...
\subsection{Cross-Method Transfer Matrix}
\label{sec:graybox_matrix}
...
% Include:
% - Reference `fig:graybox_transfer_matrix`, `fig:graybox_transfer_linf`, and `fig:graybox_transfer_l2`.
% Avoid:
...
attacks.}
\label{fig:graybox_transfer_matrix}
\end{figure}
...
Section~\ref{sec:graybox_gap} quantifies this structural inflation
...
Figures~\ref{fig:graybox_transfer_linf} and~\ref{fig:graybox_transfer_l2}
...
\label{fig:graybox_transfer_linf}
\end{figure}
...
\subsection{AttackDRO++ vs. Multi-AT}
\label{sec:graybox_headline}
...
% Include:
% - Reference `tab:paired-graybox-two-panel` and `tab:graybox_attack_family`.
% Avoid:
...
Section~\ref{sec:graybox-design}: for every triple
...
\label{tab:paired-graybox-two-panel}
...
this asymmetry in Section~\ref{sec:graybox_gap}.

\subsection{Per-Attack Analysis}
\label{sec:graybox_per_attack}
...
% Include:
% - Reference `tab:graybox_per_attack` if retained in the main flow.
% Avoid:
...
in Section~\ref{sec:evaluation_metrics} to compare the cross-method graybox
...
\label{tab:graybox_attack_family}
\small
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.08}
...
\subsection{Whitebox--Graybox Gap and Specialist Inflation}
\label{sec:graybox_gap}

% TODO[Kiệt]:
% Goal:
% - Explain the gap between direct whitebox robustness and transferred robustness.
% Flow:
% - Define the gap sign, interpret `tab:graybox_gap`, then use heatmaps for method-specific patterns.
% Include:
% - Reference gap heatmaps from `fig:graybox_gap_ddnat` to `fig:graybox_gap_attackdropp`.
...
The DDN-AT target row in Figure~\ref{fig:graybox_transfer_matrix} and
the DDN-AT row in Table~\ref{tab:graybox_attack_family} are the highest
...
Figures~\ref{fig:graybox_gap_ddnat}--\ref{fig:graybox_gap_attackdropp}
...
\label{fig:graybox_gap_attackdropp}
\end{figure}
...
\subsection{Per-(Class-Attack) Analysis}
\label{sec:graybox_per_cell}

% TODO[Kiệt]:
% Goal:
% - Explain fine-grained graybox gains and failures by class and attack.
% Flow:
% - Interpret `tab:graybox_per_cell`, then use delta heatmaps to show where gains concentrate.
% Include:
% - Reference `fig:graybox_delta_multiat`, `fig:graybox_delta_pgdat`, and `fig:graybox_delta_ddnat` if all remain in the main flow.
% Avoid:
...
Table~\ref{tab:paired-graybox-two-panel}. Cell-level significance is sparse
...
(Section~\ref{sec:per_attack_breakdown}).

The heatmaps in
Figures~\ref{fig:graybox_delta_multiat}--\ref{fig:graybox_delta_ddnat} visualize these patterns
...
Multi-AT.}
\label{fig:graybox_delta_multiat}
\end{figure}
...
\label{fig:graybox_delta_ddnat}
\end{figure}

\newpage
\subsection{Method Contribution Decomposition}
\label{sec:graybox_decomposition}
...
% Include:
% - Reference `tab:graybox_decomposition` and `fig:method_decomposition` if kept.
% Avoid:
% - Treating decomposition as causal proof beyond the available comparisons.
% Target length: 2-4 paragraphs.

The whitebox results of Section~\ref{sec:main_results} compare AttackDRO++
...
\label{tab:graybox_decomposition}
\small
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.10}
\begin{tabular}{llrrr}
...
Section~\ref{sec:graybox_headline}. Second, the Cluster-DRO effect is
...
\begin{enumerate}
  \item The graybox panel of five seeds is representative: it matches
        the full $n=20$ paired deltas to within $0.022$~pp on every
        reported metric, includes seed $7777$ (the worst AttackDRO++
        seed on the full panel), and reproduces the whitebox Mean(8)
        ranking AttackDRO++ $>$ Multi-AT $>$ PGD-AT $>$ DDN-AT to within
        $1.0$~pp.
...
Section~\ref{sec:main_results}: the AttackDRO++ improvement over
