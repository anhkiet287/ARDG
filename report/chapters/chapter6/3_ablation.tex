...
Table~\ref{tab:ch6_ablation_num_clusters} summarizes the cluster-count ablation.
Across the available runs, all settings remain close in Mean(8), with differences
...
The very fine $K=10$ setting should be interpreted cautiously because only two
valid $K=10$ runs are available in the current log.
...
\begin{figure}[H]
    ...
    \caption{Sensitivity of AttackDRO++ to the number of clusters. The figure reports Mean(8) robust accuracy with the min--max range across available runs. Mean(8) is computed over the eight non-AutoAttack evaluation attacks.}
...
Table~\ref{tab:ch6_ablation_anchor_strength} compares three anchor strengths.
...
\begin{figure}[H]
    ...
    \caption{Sensitivity of AttackDRO++ to the anchor strength $\lambda_{\mathrm{DRO}}$. The figure reports Mean(8) robust accuracy with the min--max range across available runs. Mean(8) is computed over the eight non-AutoAttack evaluation attacks. The previous $\lambda_{\mathrm{DRO}}=1.0$ setting is excluded from the final ablation panel.}
...
\begin{figure}[H]
    ...
    \caption{Sensitivity of AttackDRO++ to the cluster-refresh interval. The figure reports Mean(8) robust accuracy with the min--max range across available runs. Mean(8) is computed over the eight non-AutoAttack evaluation attacks. The two refresh schedules produce similar results, so the default two-epoch schedule is retained as a practical stability choice.}
