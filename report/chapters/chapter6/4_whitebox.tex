...
\label{sec:whitebox_class_attack}
...
\label{fig:whitebox_radar_per_attack_accuracy}
\end{figure}
...
\label{fig:whitebox_abs_multiat}
\end{figure}
...
\label{fig:whitebox_abs_attackdropp}
\end{figure}
...
\subsection{Per-Class Whitebox Robustness Pattern}
\label{sec:whitebox_per_class}
...
% Include:
% - Reference `tab:whitebox_per_cell_summary` and the delta heatmaps.
% Avoid:
...
The hardest whitebox cells are concentrated in the $\ell_\infty$ iterative attacks. For AttackDRO++, the lowest cells are PGD20-CE on \textit{cat} and \textit{deer}, followed by MI-FGSM on the same classes. This indicates that, even though AttackDRO++ improves the average multi-attack profile, the most difficult failure modes are still driven by strong $\ell_\infty$ optimization on semantically similar animal classes. In contrast, bounded $\ell_2$ attacks such as PGD-$\ell_2$, DDN-$\ell_2$, DeepFool-$\ell_2$, and CW-$\ell_2$ are generally less damaging for the proposed model than the strongest $\ell_\infty$ cells.
...
\caption{Whitebox per-(class $\times$ attack) delta of AttackDRO++ (Ours) relative to PGD-AT. Positive cells indicate where AttackDRO++ (Ours) improves over the PGD-$\ell_\infty$ specialist. The figure shows that AttackDRO++ (Ours) gains mainly on bounded $\ell_2$ attacks while PGD-AT remains stronger on several $\ell_\infty$ iterative cells.}
\label{fig:whitebox_delta_vs_pgdat}
\end{figure}
...
\label{fig:whitebox_delta_vs_ddnat}
\end{figure}
...
\label{fig:whitebox_radar_delta_vs_baselines}
\end{figure}
\vspace{-0.5cm}
...
% Include:
% - Reference `fig:whitebox_bottomk_lift` if it remains in the main flow.
% Avoid:
...
\(\ell_\infty\) attacks, especially PGD20-CE and MI-FGSM, on hard animal
...
\caption{Bottom-\(K\) whitebox class--attack lift of AttackDRO++ over each
baseline. For each baseline, the bottom-\(K\) cells are the weakest
class--attack cells of that baseline; bars report AttackDRO++ minus the
baseline on those same cells.}
\label{fig:whitebox_bottomk_lift}
\end{figure}
...
Overall, the bottom-\(K\) analysis supports the aggregate conclusion from
Section~\ref{sec:main_results}: AttackDRO++ gives a broad but modest refinement
